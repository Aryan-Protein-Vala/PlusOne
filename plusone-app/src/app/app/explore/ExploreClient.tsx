...
export default function ExplorePage({ listings }: { listings: Listing[] }) {
    ...
    <div className="app-container" style={{ maxWidth: 1000, margin: '0 auto', padding: '40px 24px' }}>
      ...
      <section style={{ marginTop: 42 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 16 }}>
          <div>
            <p style={{ margin: '0 0 6px', color: 'var(--primary)', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.08em' }}>Live marketplace</p>
            <h2 style={{ margin: 0, fontSize: 24, fontWeight: 600, letterSpacing: '-.03em' }}>People to meet in India.</h2>
          </div>
          <span style={{ color: 'var(--muted-foreground)', fontSize: 12 }}>{listings.length} active listings</span>
        </div>
        {listings.length === 0 ? (
          <div className="app-card" style={{ padding: 28, color: 'var(--muted-foreground)' }}>
            New listings are arriving soon. You can create a plan and tell the community what you want to do.
          </div>
        ) : (
              ...
              <Link key={listing.id} href={`/listings/${listing.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                <article className="app-card" style={{ padding: 20, height: '100%', transition: 'transform .2s ease' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, marginBottom: 14 }}>
                    ...
                    <span style={{ fontSize: 13, fontWeight: 700 }}>₹{Number(listing.hourly_rate).toLocaleString('en-IN')}/hr</span>
                  </div>
                  <h3 style={{ margin: '0 0 8px', fontSize: 18, fontWeight: 650 }}>{listing.title}</h3>
                  <p style={{ margin: '0 0 18px', color: 'var(--muted-foreground)', fontSize: 13, lineHeight: 1.5 }}>{listing.description}</p>
                  <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--muted-foreground)', fontSize: 12 }}>
                    <span>{listing.city}, India</span>
                    <span style={{ color: 'var(--accent-foreground)', fontWeight: 600 }}>View listing →</span>
                  </div>
                </article>
              </Link>
            ))}
